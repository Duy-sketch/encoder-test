...
int main(void)
{
  ...
  while (1)
  {
    /* USER CODE END WHILE */
	  ...
	  if (TrangThai == 1) {
	  QuayThuan();
	  sprintf(str, "THUAN: %ld \r\n", xung);
	  HAL_UART_Transmit_IT(&huart1, (uint8_t*) str, strlen(str));
	  HAL_Delay(50);
	  }
	  if (TrangThai == 2) {
	  QuayNghich();
	  sprintf(str, "NGHICH: %ld \r\n", xung);
	  HAL_UART_Transmit_IT(&huart1, (uint8_t*) str, strlen(str));
	  HAL_Delay(50);
    /* USER CODE BEGIN 3 */
  }
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pins : quay_thuan_Pin quay_nghich_Pin dung_Pin */
  GPIO_InitStruct.Pin = quay_thuan_Pin|quay_nghich_Pin|dung_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
